...
### v0.6.0
* **Премиальный дизайн Candy Glass / Cyber-Premium:** Полностью переработан пользовательский интерфейс приложения на основе макетов Stitch. Добавлены стеклянные панели (Glassmorphism) с глубоким размытием фона, динамическая неоновая подсветка с цветовым кодированием категорий трафика и плавные микро-анимации.
* **Оптимизация монитора соединений:** В списке активных соединений теперь отображаются только реально установленные TCP-соединения (`ESTABLISHED`). Избыточный опрос системных UDP-слушателей удален для снижения нагрузки на CPU и исправления проблемы переполнения счетчика соединений.

### v0.5.7
* **Правила фильтрации по маске:** Добавлена полноценная поддержка подстановочных знаков (`*` и `?`) для сопоставления правил (например, `*chrome*` или `PID_*`), позволяющая гибко группировать и проксировать схожие процессы.
* **Наивысший приоритет автозапуска:** Приоритет задачи в планировщике задач Windows (`schtasks`) повышен до максимального (`Priority 1`), а сам процесс при старте повышает свой приоритет до `HIGH_PRIORITY_CLASS` для минимизации сетевых задержек.
* **Защита от утечки трафика (ранний запуск):** Движок WinDivert и Relay-серверы теперь запускаются прямо в Rust-коде при старте Tauri-приложения. Это гарантирует мгновенный перехват трафика фоновых приложений еще до инициализации GUI.
* **Динамическая классификация процессов:** Обнаруженные процессы автоматически перегруппировываются в UI (Проксируются / Напрямую / Блокируются) в реальном времени, если они соответствуют новым или измененным правилам по маске.
* **Интерактивная памятка о масках:** Вкладка правил дополнена информационным блоком с примерами синтаксиса масок.

### v0.5.5
* **Группировка логов по процессам:** Добавлена интерактивная вкладка «По процессам» на странице системного лога, позволяющая просматривать логи и ошибки, сгруппированные по конкретным процессам в виде раскрывающегося аккордеона.
* **Фильтрация по уровням логов:** Добавлен переключатель `SegmentedControl` для фильтрации логов по уровням: «Все уровни», «Ошибки», «Предупреждения», «Инфо».
* **Исправление логов ядра:** Устранено засорение системного лога ложными записями `Error processing packet: Packet Blocked` при штатной блокировке трафика правилом `Block`. Теперь лог содержит только реальные ошибки.

### v0.5.0
* **Улучшение Changelog:** Оптимизирован парсинг и рендеринг markdown-описаний релизов в окне обновлений и в истории изменений.

### v0.4.5
* **Встроенный автоапдейтер:** Реализована полноценная система автоматического обновления приложения (проверка на старте, ручной запуск, анимированный прогресс загрузки и установка в один клик прямо из приложения).
